...
// 打包各css模块文件为一个css文件，并在index.html中Link引入：1.多次使用时，会把多个css模块文件打包到一个css文件中。2. 想打包出多个Link外部样式表时，可在此多次let引入此插件。
...
module.exports = {
  ...
  // 打包的入口文件 （相对路径）
  entry: "./src/index.js", // 可配置多个入口，对象形式，属性值为相对路径
  output: {
    ...
    // 打包后的文件夹：路径必须是个绝对路径
    path: path.resolve(__dirname, "newdist"), // __dirname代表当前路径     默认的话会在当前项目下生成dist文件夹
  },
  ...
  plugins: [
    new HtmlWebpackPlugin({
      ...
      filename: "index.html", // 打包后的html模板文件名。！！！注意不能取其他名字，否则静态服务器识别不了入口html
      // 对html模板文件的打包操作（压缩）
      minify: {
